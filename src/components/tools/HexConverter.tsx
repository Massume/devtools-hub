...
import { getHexTranslations } from '@/lib/i18n-hex';
import toast from 'react-hot-toast';

type Mode = 'encode' | 'decode';

function textToHex(text: string, uppercase: boolean, withSpaces: boolean, withPrefix: boolean): string {
  const bytes = new TextEncoder().encode(text);
  let result = Array.from(bytes)
    .map(byte => {
      let hex = byte.toString(16).padStart(2, '0');
      if (uppercase) hex = hex.toUpperCase();
      if (withPrefix) hex = '0x' + hex;
      return hex;
    })
    .join(withSpaces ? ' ' : '');
...
export function HexConverter() {
  ...
  const t = getHexTranslations(locale);
  ...
  const handleEncode = () => {
    ...
    try {
      const encoded = textToHex(input, uppercase, withSpaces, withPrefix);
      setOutput(encoded);
  ...
  const handleDecode = () => {
    ...
    try {
      const decoded = hexToText(input.trim());
      setOutput(decoded);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Format Options (only for encode mode) */}
      {mode === 'encode' && (
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                className="w-4 h-4 rounded border-hub-border bg-hub-darker text-hub-accent focus:ring-hub-accent"
              />
              <span className="text-sm text-gray-300">{t.withSpaces}</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                ...
                className="w-4 h-4 rounded border-hub-border bg-hub-darker text-hub-accent focus:ring-hub-accent"
              />
              <span className="text-sm text-gray-300">{t.withPrefix}</span>
            </label>
          </div>
        </div>
      )}
