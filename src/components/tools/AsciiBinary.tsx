...
import { getAsciiBinaryTranslations } from '@/lib/i18n-ascii-binary';
import toast from 'react-hot-toast';

type Mode = 'encode' | 'decode';

function textToBinary(text: string, withSpaces: boolean): string {
...
function binaryToText(binary: string): string {
  ...
  if (!/^[01]+$/.test(cleanBinary)) {
    throw new Error('Invalid binary: contains non-binary characters');
  }
  ...
  for (let i = 0; i < cleanBinary.length; i += 8) {
    bytes.push(cleanBinary.substr(i, 8));
  }
...
export function AsciiBinary() {
  ...
  const t = getAsciiBinaryTranslations(locale);
  ...
  const handleEncode = () => {
    ...
    try {
      const encoded = textToBinary(input, withSpaces);
      setOutput(encoded);
