...
import { getHtmlEntitiesTranslations } from '@/lib/i18n-html-entities';
...
function encodeHtmlEntities(text: string, encodeAll: boolean): string {
  if (encodeAll) {
...
export function HtmlEntities() {
  ...
  const t = getHtmlEntitiesTranslations(locale);
  ...
  const handleEncode = () => {
    ...
    try {
      const encoded = encodeHtmlEntities(input, encodeAll);
      setOutput(encoded);
    ...
    <div className="space-y-6">
      ...
      {mode === 'encode' && (
        <div className="bg-hub-card border border-hub-border rounded-xl p-4">
          <h3 className="text-sm font-semibold text-hub-muted mb-3">{t.encodeOptions}</h3>
          <div className="flex flex-wrap gap-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                name="encodeMode"
                checked={encodeAll}
                onChange={() => setEncodeAll(true)}
                className="w-4 h-4 border-hub-border bg-hub-darker text-hub-accent focus:ring-hub-accent"
              />
              <span className="text-sm text-gray-300">{t.encodeAll}</span>
            </label>
          </div>
        </div>
      )}
