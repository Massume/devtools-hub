...
import { getJsFormatterTranslations } from '@/lib/i18n-js-formatter';
...
export function JsFormatter() {
  ...
  const t = getJsFormatterTranslations(locale);
  ...
  const formatJs = async () => {
    ...
    try {
      const formatted = await prettier.format(input, {
        parser: 'babel',
        plugins: [babelParser as Plugin, estreePlugin as Plugin],
        tabWidth: indentSize,
        semi: semicolons,
      ...
      {mode === 'format' && (
        <div className="bg-hub-card border border-hub-border rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-sm text-hub-muted mb-1">{t.semicolons}</label>
              <select
                value={semicolons ? 'yes' : 'no'}
                onChange={(e) => setSemicolons(e.target.value === 'yes')}
                className="w-full bg-hub-darker border border-hub-border rounded-lg px-3 py-2 text-white focus:outline-none focus:border-hub-accent"
              >
            ...
            <div>
              <label className="block text-sm text-hub-muted mb-1">{t.singleQuote}</label>
              <select
                value={singleQuote ? 'yes' : 'no'}
                onChange={(e) => setSingleQuote(e.target.value === 'yes')}
                className="w-full bg-hub-darker border border-hub-border rounded-lg px-3 py-2 text-white focus:outline-none focus:border-hub-accent"
              >
            ...
            <div>
              <label className="block text-sm text-hub-muted mb-1">{t.trailingComma}</label>
              <select
                value={trailingComma}
                onChange={(e) => setTrailingComma(e.target.value as 'none' | 'es5' | 'all')}
                className="w-full bg-hub-darker border border-hub-border rounded-lg px-3 py-2 text-white focus:outline-none focus:border-hub-accent"
              >
